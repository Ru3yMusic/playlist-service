 ...
 *
 * TDD cycle:
 *   RED  — JwtAuthenticationFilter does not exist yet → compile fails
 *   GREEN — class created, all cases pass
 */
...
class JwtAuthenticationFilterTest {
    ...
    void doFilter_validServiceToken_setsServiceRoleAuthentication() throws Exception {
        ...
        when(claims.getSubject()).thenReturn("interaction-service");
        ...
        assertThat(auth.getName()).isEqualTo("interaction-service");
    ...
    void doFilter_validUserToken_setsUserRoleAuthentication() throws Exception {
        Claims claims = mock(Claims.class);
        when(claims.getSubject()).thenReturn("user-xyz-789");
        ...
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        assertThat(auth).isNotNull();
        assertThat(auth.getName()).isEqualTo("user-xyz-789");
        assertThat(auth.getAuthorities())
                .extracting(a -> a.getAuthority())
